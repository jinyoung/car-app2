...
func (self *PolicyHistory) Get(c echo.Context) error {
	repository := PolicyHistoryRepository()
	...
	embeddedRel, _ := hal.NewResourceRelation("policyHistories")
...
func (self *PolicyHistory) FindById(c echo.Context) error{
	repository := PolicyHistoryRepository()
...
func (self *PolicyHistory) Persist(c echo.Context) error{
	repository := PolicyHistoryRepository()
...
func (self *PolicyHistory) Put(c echo.Context) error{
	repository := PolicyHistoryRepository()
	id, _ := strconv.Atoi(c.Param("id"))
	params := make(map[string]string)
...
func (self *PolicyHistory) Remove(c echo.Context) error{
	repository := PolicyHistoryRepository()
	id, _ := strconv.Atoi(c.Param("id"))
	self, err := repository.FindById(id)
	if err != nil {
